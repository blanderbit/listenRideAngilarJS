...
angular.module('listnride').factory('price', ['$translate',
  function($translate) {
    var durationDays = function(startDate, endDate) {
    ...
    return {
      calculatePrices: function(startDate, endDate, prices) {
        if (startDate === undefined || endDate === undefined) {
          ...
          var days = durationDays(startDate, endDate);
          if (days < 8) {
            return prices[days - 1];
          } else {
            return prices[6] + (days - 7) * prices[7];
          }
        }
      },
